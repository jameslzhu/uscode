...
def run(options):
    ...
    filenames = glob.glob("data/uscode.house.gov/xhtml/{}/{}.htm".format(year, title))
    filenames.sort()
...
def download_usc(year, options):
    ...
    dest_dir = "data/uscode.house.gov/xhtml/{}".format(year)
    ...
    # -q: quiet, -m: mirroring -l1 leve 1 -P prefix "data"
    # os.system("wget -q -m -l1 -P %s http://uscode.house.gov/xhtml/%s" % ("data", year))
    release= (115, 185)
    url = 'http://uscode.house.gov/download/releasepoints/us/pl/{0}/{1}/htm_usc01@{0}-{1}.zip'.format(*release)
    r = requests.get(url)
    z = zipfile.ZipFile(io.BytesIO(r.content))
    with open('usc01@{}-{}'.format(*release), 'r') as f:
        f.write(z.extractall())